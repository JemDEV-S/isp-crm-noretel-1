...
@section('title', 'Nueva Plantilla de Notificación')

@section('page-title', 'Nueva Plantilla de Notificación')

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">Crear Plantilla de Notificación</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('core.notifications.templates.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('variables') is-invalid @enderror" id="variables" name="variables" value="{{ old('variables') ? old('variables') : '{nombre},{email},{fecha}' }}">
                @error('variables')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('language') is-invalid @enderror" id="language" name="language" required>
                    ...
                    @foreach($languages as $lang)
                        <option value="{{ $lang }}" {{ old('language', 'es') == $lang ? 'selected' : '' }}>
