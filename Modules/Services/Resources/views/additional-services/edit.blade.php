...
@section('page-title', 'Editar Servicio Adicional: ' . $additionalService->name)
...
@section('actions')
    <div class="btn-group" role="group">
        ...
        <a href="{{ route('services.additional-services.show', $additionalService->id) }}" class="btn btn-sm btn-info">
    ...
    <div class="row">
        <div class="col-lg-8 mx-auto">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('services.additional-services.update', $additionalService->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('service_id') is-invalid @enderror" id="service_id" name="service_id" required>
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" {{ old('service_id', $additionalService->service_id) == $service->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" min="0" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price', $additionalService->price) }}" required>
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $additionalService->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="configurable" name="configurable" value="1" {{ old('configurable', $additionalService->configurable) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3 configuration-options" style="{{ old('configurable', $additionalService->configurable) ? '' : 'display: none;' }}">
                            <label class="form-label">Opciones de configuración</label>
                            <div class="mb-2">
                                <div class="row option-inputs">
                                    @php
                                        $options = old('configuration_options', $additionalService->configuration_options) ?: [];
                                    @endphp
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="{{ route('services.additional-services.show', $additionalService->id) }}" class="btn btn-secondary me-md-2">Cancelar</a>
                            <button type="submit" class="btn btn-primary">Actualizar Servicio Adicional</button>
                        </div>
                    </form>
                </div>
            </div>
